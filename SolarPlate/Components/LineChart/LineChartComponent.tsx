...
const LineChartComponent: React.FC<LineChartProps> = ({ yAxisLabel, yAxisSuffix, chartTitle, data, chartXData, chartStyle, titleStyle }) => {
  ...
  useEffect(() => {
    setChartData(data.slice(-7)); // Limita aos últimos 5 registros
  }, [data]);

  // Extrai apenas o horário das strings de data
  const hours = chartXData.map((dateTime) => new Date(dateTime).toLocaleTimeString().slice(0, 7)).slice(-7); // Limita aos últimos 5 registros
